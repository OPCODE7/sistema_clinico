...
Public Class FrmDescuento
    ...
    Private Function Validator() As Boolean
        ...
        If TxtDescuento.Text.Trim().Length = 0 Then
            MessageBox.Show("Por favor, ingrese la cantidad de descuento.", "ATENCION", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            TxtDescuento.Focus()
            isvalid = False
        ElseIf TxtDescripcion.Text.Trim().Length = 0 Then
            MessageBox.Show("Por favor, ingrese la descripcion del descuento.", "ATENCION", MessageBoxButtons.OK, MessageBoxIcon.Warning)
    ...
    Private Function ObtenerSiguienteID() As Integer
        ...
        Using connection As New SqlConnection(connectionString)
            ...
            Dim query As String = "SELECT ISNULL(MAX(IdDescuento), 0) + 1 FROM Descuento"
    ...
    Private Sub GroupDescuento_Enter(sender As Object, e As EventArgs) Handles GroupDescuento.Enter
    End Sub
    ...
    Private Sub BtnGuardar_Click(sender As Object, e As EventArgs) Handles BtnGuardar.Click
        If Validator() Then
            ...
            Dim cmd As New SqlClient.SqlCommand("sp_Descuento", con)
            cmd.CommandType = CommandType.StoredProcedure

            cmd.Parameters.AddWithValue("@Identificador", 1)
            cmd.Parameters.AddWithValue("@IdDescuento", ObtenerSiguienteID)
    ...
    Private Sub PanelDescuento_Paint(sender As Object, e As PaintEventArgs) Handles PanelDescuento.Paint
    End Sub

    Private Sub ImagenBuscar_Click(sender As Object, e As EventArgs) Handles ImagenBuscar.Click
        con.Open()

        Dim cmd As New SqlClient.SqlCommand("Select Descuento.IdDescuento, Descuento.Descuento, Descuento.Descripcion, Descuento.FechaRegistro, Descuento.Estado
                                             From Descuento
                                             WHERE Descripcion = '" & TxtBuscar.Text & "'", con)
        ...
        If reader.Read Then
            ...
            TxtId.Text = reader("IdDescuento").ToString()
    ...
    '    If String.IsNullOrWhiteSpace(TxtBuscar.Text) OrElse TxtBuscar.Text = "Ingrese descuento" Then
    '        TxtBuscar.Text = ""
    ...
    '        TxtBuscar.Text = "Ingrese descuento"
    ...
    Private Sub BtnActualizar_Click(sender As Object, e As EventArgs) Handles BtnActualizar.Click
        If Validator() Then
            ...
            Dim cmd As New SqlClient.SqlCommand("UPDATE Descuento
                                                 SET Descuento = @Descuento, Descripcion = @Descripcion, FechaModificacion = GETDATE(), Estado = @Estado
                                                 WHERE IdDescuento = @IdDescuento", con)

            cmd.Parameters.AddWithValue("@IdDescuento", TxtId.Text)
            cmd.Parameters.AddWithValue("@Descuento", TxtDescuento.Text)
            cmd.Parameters.AddWithValue("@Descripcion", TxtDescripcion.Text)
            cmd.Parameters.AddWithValue("@FechaModificacion", DateFechaRegistro.Value)
            cmd.Parameters.AddWithValue("@Estado", If(CheckActivo.Checked, 1, 0))
    ...
    Private Sub BtnEliminar_Click(sender As Object, e As EventArgs) Handles BtnEliminar.Click
        ...
        Dim cmd As New SqlClient.SqlCommand("DELETE 
                                             FROM Descuento
                                             WHERE IdDescuento = '" + TxtId.Text + "'", con)
    ...
    Private Function ObtenerSugerencias(textoBusqueda As String) As List(Of String)
        ...
        Dim query As String = "SELECT Descripcion FROM Descuento WHERE Descripcion LIKE '%' + @textoBusqueda + '%'"
        ...
        Using connection As New SqlConnection(connectionString)
            ...
            Using command As New SqlCommand(query, connection)
                ...
                Using reader As SqlDataReader = command.ExecuteReader()
                    While reader.Read()
                        Dim sugerencia As String = reader("Descripcion").ToString()
    ...
    Private Sub GroupDescuento_ControlAdded(sender As Object, e As ControlEventArgs) Handles GroupDescuento.ControlAdded
        ClearForm()
        StartForm()
    End Sub
End Class